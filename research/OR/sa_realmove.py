#!/usr/bin/env python3
"""
Simulated Annealing (SA) planner using game-like movement.

Goal: minimize total sailed distance while reaching the mark.
Approach: at each step, compare "tack now" vs "no tack" over a short horizon.
Unlike MPC which is purely greedy, SA can accept worse moves with a probability
that decreases over time (temperature cooling), allowing escape from local optima.
...
DEFAULT_MIN_TEMP = 0.1
DEFAULT_REHEAT_FACTOR = 1.0
...
def sa_plan_real(
    ...
    while step_count < len(wind_speed):
        ...
        if cost_b < cost_a:
            # Tack is better -> always accept
            do_tack = True
        elif cost_b < float("inf") and temp > min_temp:
            # Tack is worse -> accept with SA probability
            delta = cost_b - cost_a
        ...
        # Reheat if stagnating (optional, when reheat_factor > 1.0)
        current_cost = min(cost_a, cost_b)
        if prev_cost is not None and abs(current_cost - prev_cost) < 1e-3:
            stagnation += 1
        else:
            stagnation = 0
        if reheat_factor > 1.0 and stagnation > 50:
            temp = min(temp * reheat_factor, initial_temp)
            stagnation = 0
...
def main():
    parser = argparse.ArgumentParser(description="Simulated Annealing (SA) planner with game-like movement.")
    ...
    parser.add_argument("--initial-temp", type=float, default=DEFAULT_INITIAL_TEMP, help="Initial temperature")
    parser.add_argument("--cooling-rate", type=float, default=DEFAULT_COOLING_RATE, help="Temperature cooling rate per step")
    parser.add_argument("--min-temp", type=float, default=DEFAULT_MIN_TEMP, help="Minimum temperature threshold")
    parser.add_argument("--reheat-factor", type=float, default=DEFAULT_REHEAT_FACTOR, help="Reheat multiplier on stagnation (1.0 = disabled)")
